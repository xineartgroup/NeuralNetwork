        ...
        public static (List<double>, bool) LoadAndProcessImage(string imagePath)
        {
            try
            {
                ...
                if (bitmap.Width != ImageSize || bitmap.Height != ImageSize)
                {
                    ...
                    workingBitmap = new Bitmap(ImageSize, ImageSize);
                ...
                BitmapData data = workingBitmap.LockBits(rect, ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
                ...
                workingBitmap.Dispose(); // Explicitly dispose since we can't use 'using' with the conditional assignment
